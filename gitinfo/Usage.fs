// (c) 2026  ttelcl / ttelcl
module Usage

open CommonTools
open ColorPrint

let usage focus =
  let showSection section =
    focus = "" || focus = section || focus = "all"
  let showDetail section =
    focus = section || focus = "all"
  if showSection "hash" then
    cp "\fogitinfo \fyhash [\fg-t \fctype\f0] [\fg-fb \fcfile\f0|\fg-ft \fcfile\f0|\fg-s \fctext\f0|\fg-l \fcline\f0]"
    cp "   Calculate the git hash for a blob or other object."
  if showDetail "hash" then
    cp "  \fg-t \fctype\f0       The type of git object. Defaults to '\foblob\f0'."
    cp "  \fg-fb \fcfile\f0      The binary file whose content is the blob (or object)"
    cp "  \fg-ft \fcfile\f0      The text file whose content is the blob (or object) - after replacing CRLF pairs with LF"
    cp "  \fg-s \fctext\f0       The literal text content forming the blob content (as UTF8)"
    cp "  \fg-l \fcline\f0       Like \fg-s\f0, but with a LineFeed character (\fo\\n\f0) appended."
    cp ""
  if showSection "stamp" then
    cp "\fogitinfo \fystamp\f0 [\fg-repo \fcpath\f0] [\fg-ref \fcref\f0] {\fg-t \fcfile\f0} {\fg-T \fcfile\f0} [\fg-a\f0]"
    cp "   Retrieve the time stamp of a git branch/tag/ref/commit and optionally touch a file with it"
  if showDetail "stamp" then
    cp "  \fg-repo \fcpath\f0    Any path within the target repository. Defaults to the current directory"
    cp "  \fg-ref \fcref\f0      The commit (or reference, such as branch or tag) to get the commit stamp for"
    cp "  \fx\fx\fx              Defaults to the currently checked out commit (and has no default if there is none)"
    cp "  \fg-t \fcfile\f0       Create or touch the given file and set its stamp to the commit stamp. Repeatable."
    cp "  \fg-T \fcfile\f0       Like \fg-t\f0, but if the file already exists, ignore the commit stamp if it is older"
    cp "  \fg-a\f0\fx            If given: use the authored stamp instead of the committed stamp"
    cp ""
  if showSection "references" then
    cp "\fogitinfo \fyreferences\f0 [\fg-repo \fcpath\f0] [\fg-list\f0] [\fg-dump\f0]"
    cp "  \fk(alias: \fogitinfo \fyrefs\fk)\f0"
    cp "   List all references in the git repository"
  if showDetail "references" then
    cp "  \fg-repo \fcpath\f0    Any path within the target repository. Defaults to the current directory"
    cp "  \fg-list\fx\f0         List references to the console"
    cp "  \fg-dump\fx\f0         Dump references to a CSV file (name derived from the repository)"
    cp ""
  if showSection "commits" then
    cpx "\fogitinfo \fycommits\f0 [\fg-repo \fcpath\f0] {\fg-i \fcinclude-glob\f0} {\fg-x \fcexclude-glob\f0}"
    cp " [\fg-dump\f0] [\fg-tips\f0] [\fg-edges\f0] [\fg-graph\f0 [\fGoptions\f0]] [\fg-show\f0] [\fg-list \fcn\f0]"
    cp "   Return information on commits in the git repository matching the include and exclude criteria."
    cp "   Without any \fg-i\f0 options, commits on the current branch are selected."
  if showDetail "commits" then
    cp "  \fg-repo \fcpath\f0    Any path within the target repository. Defaults to the current directory"
    cp "  \fg-label \fclabel\f0  Change the base name of generated files from the repository name to \fclabel\f0."
    cp "  \fg-dump\fx\f0         Dump per-commit information to a CSV file (name derived from the repository or \fg-label\f0)"
    cp "  \fg-edges\fx\f0        Dump a CSV file describing the inter-commit edges (name derived from the repository or \fg-label\f0)"
    cp "  \fg-tips\fx\f0         Dump information on tip and tail commits to a CSV file (name derived from the repository or \fg-label\f0)"
    cp "  \fg-graph\f0\fx        Dump a graphops.exe compatible graph model of the selected commits and their edges (name derived from the repository or \fg-label\f0)"
    cp "    \fg-name\f0\fx       Include user names of author and/or committer in \fg-graph\f0 output."
    cp "    \fg-email\f0\fx      Include user emails of author and/or committer in \fg-graph\f0 output."
    cp "    \fg-no-message\f0\fx Exclude the commit message header from \fg-graph\f0 output."
    cp "    \fg-no-sha\f0\fx     Exclude the full commit SHA from \fg-graph\f0 output."
    cp "  \fg-show\fx\f0         Show relevant commits on console"
    cp "  \fg-list \fcn\f0       List the first \fcn\f0 matching commits on the console"
    cp "  \fg-i \fcinclude\f0    A glob expression to include (e.g. '\fg-i \fcrefs/heads/*\f0' to include all local branches)"
    cp "  \fg-x \fcexclude\f0    A glob expression to exclude (e.g. '\fg-x \fcrefs/remotes/origin/*\f0' to exclude all commits on remote branches)"
    cp ""
  if showSection "chain" then
    cp "\fogitinfo \fychain\f0 [\fg-repo \fcpath\f0] {[\fg-from\f0|\fg-to\f0|\fg-both\f0] \fccommit\f0}"
    cp "   List the chain of 1-child or 1-parent commits leading up to or up from or both the commit"
  if showDetail "chain" then
    cp "  \fx      \fccommit\f0  Commits can be specified as commit ids or refs"
    cp "  \fg-repo \fcpath\f0    Any path within the target repository. Defaults to the current directory"
    cp "  \fg-from \fccommit\f0  Walk up along children of \fccommit\f0 until the first commit that has not exactly 1 child"
    cp "  \fg-to   \fccommit\f0  Walk down along parents of \fccommit\f0 until the first commit that has not exactly 1 parent"
    cp "  \fg-both \fccommit\f0  Combine \fg-from\f0 and \fg-to\f0, walking the commit chain on both sides"

  cp "\fg-v               \f0Verbose mode"



